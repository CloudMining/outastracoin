...
string CRPCTable::help(string strCommand) const
{
    ...
    vector<pair<string, const CROACommand*> > vCommands;

    for (map<string, const CROACommand*>::const_iterator mi = mapCommands.begin(); mi != mapCommands.end(); ++mi)
        vCommands.push_back(make_pair(mi->second->category + mi->first, mi->second));
    sort(vCommands.begin(), vCommands.end());

    BOOST_FOREACH(const PAIRTYPE(string, const CROACommand*)& command, vCommands)
    {
        const CROACommand *pcmd = command.second;
 ...
 * Call Table
 */
static const CROACommand vROACommands[] =
...
{
    unsigned int vcidx;
    for (vcidx = 0; vcidx < (sizeof(vROACommands) / sizeof(vROACommands[0])); vcidx++)
    {
        const CROACommand *pcmd;

        pcmd = &vROACommands[vcidx];
        mapCommands[pcmd->name] = pcmd;
    }
}

const CROACommand *CRPCTable::operator[](string name) const
{
    map<string, const CROACommand*>::const_iterator it = mapCommands.find(name);
...
{
    // Find method
    const CROACommand *pcmd = tableRPC[strMethod];
    if (!pcmd)
