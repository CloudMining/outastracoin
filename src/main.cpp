...
uint256 CPartialMerkleTree::ExtractMatches(std::vector<uint256> &vMatch) {
    ...
    if (nTransactions > MAX_BLOCK_SIZE / 100) // 100 is the lower bound for the size of a serialized CTransaction
        return 0;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x75d6778f3365e015cbc4da5b888b137c856e18ad5bbc99de16c27112779bd557");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 488804799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        ...
        block.nVersion = 1;
        block.nTime    = 1481540255;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 443092;

        if (fTestNet)
        {
            block.nTime    = 1481540201;
            block.nNonce   = 53165757;
        }
...
unsigned char pchMessageStart[4] = { 0x70, 0x35, 0x22, 0x05 }; // Outastracoin: increase each by adding 2 to bitcoin's value.
